...
date = (1,1)
comments = (1,21)
Summarydata = [(2,3),(2,4),(2,6),
           (2,7),(2,8),(2,10),(2,11),(2,13),
           (2,14),(2,16),(2,17),(2,19),(2,20)]

fueldata = [(7,43),(7,45)]

avgs1 = [(33,12),(1,1500),(2,1500),(3,1500),(4,1500),   #Barrometric PreselectedSheeture,Train1 CF,Train1 cfm,Train1 dH,Train1 Temp
...
try:
    with open(data,'w') as f:
        ...
        f.write("Date,comments,Time (min),Time (hrs),BTU's per hr,High Efficiency, Low Efficiency,"+
                "Train1 gr/hr,Train1 Lbs/mmBTU,Train2 gr/hr,Train2 Lbs/mmBTU,Amb gr/hr,Amb Lbs/mmBTU,Total gr/hr,Total Lbs/mmBTU," +
                "Load Weight,Load MC,"+
                "Barrometric PreselectedSheeture,Train1 CF,Train1 cfm,Train1 dH,Train1 Temp," +
                "Amb CF,Amb cfm,Amb dH,Amb Temp," +
                "Tunnel Temp, Tunnel dP," +
                "Load Flow,Boiler Flow," +
                "Boiler End Temp,Boiler Out,Boiler In,Load In,Load Out," +
    ...
    for file in files:
        ...
        print("Opening " + file)
        try:
            doc = openspreadsheet(file)

            selectedSheet = getsheetref(doc, 'Summary')
    
            row = getcellstring(selectedSheet, date)
            row = row + ',"'+ getcellstring(selectedSheet,comments) + '"'

            for cell in Summarydata:
                row = row + ', ' + getcellstring(selectedSheet,cell)
    
            selectedSheet = getsheetref(doc,'I Fuel')
            for cell in fueldata:
                row = row + ',' + getcellstring(selectedSheet,cell)

            selectedSheet = getsheetref(doc,'I Data')

            if getcellstring(selectedSheet,(0,257)) == 'Avg/Total':
                avgs= avgs3
            elif getcellstring(selectedSheet,(0,425)) == 'Avg/Total':
                avgs = avgs4
            elif getcellstring(selectedSheet,(0,1000)) == 'Avg/Total':
                avgs = avgs2
            else:
                avgs = avgs1
            ...
            selectedSheet = getsheetref(doc, 'I Lab')
            for cell in labs:
                row = row +','+  getcellstring(selectedSheet,cell)
            ...
            exceptionprint(e)
            row = row + ' file structure error: ' + file
